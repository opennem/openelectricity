...
export default function () {
	...
	const orderedModelScenarioPathways = derived(
		selectionData,
		(/** @type {any[]} */ $selectionData) => {
			/** @type {any[]} */
			let orderModels = [];

			Object.keys(modelScenarios).forEach((model) => {
				const findModel = $selectionData.find(
					(/** @type {any} */ selection) => selection.model === model
				);
				...
				if (findModel) {
					...
					const filterPathways = $selectionData.filter(
						(/** @type {any} */ selection) => selection.model === model
					);
					const scenarioOrder = /** @type {Record<string, any>} */ (modelScenarios)[model].map(
						(/** @type {{ id: string }} */ { id }) => id
					);
					const scenarios = [
						...new Set(filterPathways.map((/** @type {any} */ pathway) => pathway.scenario))
					];
					const returnedOrder = scenarioOrder.filter((/** @type {string} */ scenario) =>
						scenarios.includes(scenario)
					);
					ordered.scenarios = returnedOrder.map((/** @type {string} */ scenario) => {
						const pathways = filterPathways.filter(
							(/** @type {any} */ pathway) => pathway.scenario === scenario
						);
