...
<div
	class="shrink-0 border-t border-warm-grey px-5 py-4 tablet:py-2 flex items-center gap-3 text-xxs text-mid-grey bg-white"
>
	<Building2 size={14} class="shrink-0 hidden tablet:block" />
	<span class="min-w-0 flex-1 truncate hidden tablet:block">
		{#if owners.length}
	...
	{#if facilityCode}
		<a
			...
			class="group shrink-0 inline-flex items-center justify-center gap-2 w-full tablet:w-auto px-4 py-2 text-sm font-semibold rounded-lg shadow-sm transition-all hover:opacity-90 hover:shadow no-underline hover:no-underline {darkText
				? 'text-black'
				: 'text-white'}"
		>
