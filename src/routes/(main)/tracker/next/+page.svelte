<script>
	/**
	 * Tracker — the canonical-to-be tracker page.
	 *
	...
	function syncUrl(mode = 'replace') {
		...
		if (mode === 'push') pushState(`${resolve('/(main)/tracker/next')}${url.search}`, {});
		else replaceState(`${resolve('/(main)/tracker/next')}${url.search}`, {});
	}
...
<FullscreenLayout {isFullscreen} gridline={false} class="bg-light-warm-grey">
	{#snippet filterBar()}
		<div class="relative z-40 shrink-0 border-b border-warm-grey {isFullscreen ? '' : 'px-4'}">
			<FullscreenFilterBar
				{isFullscreen}
				routeKey="tracker-next"
				stableName="filter-bar-stable-tracker-next"
				paddingX="px-8"
			...
			>
				{#snippet stable()}
					{#if isFullscreen}
						<FullscreenNavDropdown />
						<a
							href={resolve('/(main)/tracker/next')}
