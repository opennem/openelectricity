/**
 * 7-Day Tracker API — OE API source (comparison variant of /7d-processed).
 *
 * Fetches NEM power by fueltech from the OE API at 5-minute resolution for the
 * last 7 days, then runs the *same* `processPower7d` pipeline as the static-JSON
 * endpoint so the two charts differ only by data source.
 *
 * Notes:
 *  - The OE 30m interval is rejected ("Unprocessable Entity"), so we fetch 5m
 *    and aggregate to the requested interval ourselves (matching the JSON path).
 *  - The OE API expects timezone-naive dates in NEM-local time (+10:00), so the
 *    date window is offset before being sent.
 *  - Unlike the pre-aligned static JSON, OE returns each fueltech on its own
 *    window (different start/length) and 30-minute series (rooftop solar) are
 *    step-held across 5m with nulls in some slots. Both are normalised by
 *    `alignAndFillPowerSeries` before the shared processor, otherwise the
 *    index-based time reconstruction misaligns series and `groupData`'s
 *    null→0 turns the rooftop gaps into visible dips.
 */
...
/**
 * Normalise OE per-fueltech power series so they look like the pre-aligned
 * static JSON the shared processor expects:
...
export async function GET({ url }) {
	...
	try {
		...
		// Drop OE's net `battery` series (= battery_discharging − battery_charging).
		// It's redundant with the charging/discharging split the chart already
		// stacks, so keeping it would double-count battery. The static JSON doesn't
		// return it. See the "Battery" note in the file header / README.
