<script>
	 ...
	 * @property {any} cover
	 * @property {Array<import('$lib/types/article.types').Author & { bio?: string }> | null} author
	 * @property {any} tldr
	...
	/** @type {Props} */
	let { data, preview = false } = $props();

	let hasCover = $derived(data.cover);
	let publishedDate = $derived(parse(data.publishDate ?? '', 'yyyy-MM-dd', new Date()));
	let formattedDate = $derived(
		isValid(publishedDate) ? format(publishedDate, 'do MMM, yyyy') : null
	);
	let authors = $derived(data.author ?? []);
</script>
...
<div class="relative">
	...
	<div class="container max-w-none lg:container py-12 relative z-10">
		<div class="pb-10 md:py-14">
			<a
				href={resolve(preview ? '/article-drafts' : '/analysis')}
			...
			{#each authors as author, i (i)}
				<span class={`w-20 h-20 block grayscale relative ${i > 0 ? '-left-3' : 'left-0'}`}>
					{#if author.image}
						...
						/>
					{:else}
						<img class="rounded-full border border-white p-2" src="/favicon.png" alt="author" />
					{/if}
				</span>
			{/each}
		</div>
