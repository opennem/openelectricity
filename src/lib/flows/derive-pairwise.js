 ...
 *
 * The derivation is linear, so it holds for the energy variants too — pass
 * `flow_imports_energy`/`flow_exports_energy` via the metric options.
 *
...
export function derivePairwiseFlows(
	apiData,
	{ importsMetric = 'flow_imports', exportsMetric = 'flow_exports' } = {}
) {
	const imports = collectRegionSeries(apiData, importsMetric);
	const exports_ = collectRegionSeries(apiData, exportsMetric);
