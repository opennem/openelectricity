/**
 * Rune-based live grid data store for the tracker map.
 *
...
export function createGridLive() {
	...
	async function fetchLatest() {
		try {
			...
			console.error('[grid-live] live grid refresh failed:', e);
		}
	}
	...
	/** Hidden tabs skip polls; returning to the tab catches up immediately
	 *  (the in-flight dedupe absorbs any overlap with a due interval tick). */
	function handleVisibilityChange() {
		if (!document.hidden) refresh();
	}

	/** Immediate refresh, then poll every 5 minutes while visible. Idempotent. */
	function start() {
