...
describe('derivePairwiseFlows', () => {
	...
	it('derives from the energy metric pair when configured', () => {
		const energySample = [
			metricEntry('flow_imports_energy', { QLD1: [[T1, 85.5]] }),
			metricEntry('flow_exports_energy', { QLD1: [[T1, 12.25]] })
		];
		const { series } = derivePairwiseFlows(energySample, {
			importsMetric: 'flow_imports_energy',
			exportsMetric: 'flow_exports_energy'
		});
		expect(series['NSW1->QLD1'][0]).toBeCloseTo(73.25, 4);
	});
