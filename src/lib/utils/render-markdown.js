...
export function renderMarkdown(text) {
	...
	html = html.replace(
		/```[\w]*\n([\s\S]*?)```/g,
		'<pre class="bg-light-warm-grey rounded px-2 py-1 my-1 overflow-x-auto">$1</pre>'
	);

	// Tables — detect lines with | separators
	html = html.replace(/((?:^.*\|.*$\n?){2,})/gm, (/** @type {string} */ tableBlock) => {
		...
		dataRows.forEach((row, idx) => {
			...
			const cls =
				tag === 'th'
					? ' class="text-left pr-3 text-dark-grey border-b border-warm-grey pb-0.5"'
	...
	html = html.replace(/((?:^[ \t]*[-*] .+$\n?)+)/gm, (/** @type {string} */ block) => {
		const items = block
			.trim()
			.split('\n')
			.map((l) => l.replace(/^[ \t]*[-*] /, ''));
		return (
			'<ul class="my-0.5 ml-3 list-disc">' + items.map((i) => `<li>${i}</li>`).join('') + '</ul>'
		);
	});
	...
	html = html.replace(/((?:^[ \t]*\d+\. .+$\n?)+)/gm, (/** @type {string} */ block) => {
		const items = block
			...
			.map((l) => l.replace(/^[ \t]*\d+\. /, ''));
		return (
			'<ol class="my-0.5 ml-3 list-decimal">' + items.map((i) => `<li>${i}</li>`).join('') + '</ol>'
		);
	});
